...
q1_price <- quantile(dane$price, 0.25)
q3_price <- quantile(dane$price, 0.75)
iqr_price <- q3_price - q1_price
min_price <- min(dane$price)
max_price <- max(dane$price)

# Tworzenie tabeli
tabela <- data.frame(
  Statystyka = c("Mediana", "Kwartyl 1", "Kwartyl 3", "Rozstęp międzykwartylowy", "Minimum", "Maksimum"),
  `1 pokój` = c(median_price, q1_price, q3_price, iqr_price, min_price, max_price),
  `2 pokoje` = c(median_price, q1_price, q3_price, iqr_price, min_price, max_price),
  `3 pokoje` = c(median_price, q1_price, q3_price, iqr_price, min_price, max_price),
  `4 pokoje` = c(median_price, q1_price, q3_price, iqr_price, min_price, max_price)
)
...
W badaniu dotyczącym cen nieruchomości przeprowadzono szereg testów statystycznych, mających na celu zbadanie związków między cenami a różnymi zmiennymi, w tym powierzchnią mieszkania. Kluczowe wyniki obejmują testy normalności, testy Wilcoxona oraz analizę korelacji, które pomogły zrozumieć wzajemne relacje między analizowanymi zmiennymi.
...
\`\`\`{r} \# Utworzenie ramki danych zawierającej kolumny z ceną i
kategoriami udogodnień df \<- data.frame( price =
dane$price,  mainroad = as.factor(dane$mainroad), prefarea =
as.factor(dane$prefarea),  basement = as.factor(dane$basement),
hotwaterheating =
as.factor(dane$hotwaterheating),  airconditioning = as.factor(dane$airconditioning),
parking =
as.factor(dane$parking),  furnishingstatus = as.factor(dane$furnishingstatus)
)
...
ggplot(df, aes(x = interaction(mainroad, prefarea, basement,
hotwaterheating, airconditioning, parking, furnishingstatus), y =
price)) + geom_boxplot(fill = "lightblue") + labs(title = "Zależność
ceny od różnych udogodnień", x = "Udogodnienia", y = "Cena") +
