...
library(arules) library(hms) library(readxl) library(ggplot2)
library(dplyr) library(ggstatsplot) library(magrittr) library(gt)
library(lubridate) library(gtsummary) library(summarytools)
library(qwraps2) library(kableExtra)
...
plot(dane$area, dane$price, main="Rozrzut cen vs. powierzchnia",
xlab="Powierzchnia", ylab="Cena") 
outliers <- boxplot(data$price ~ data$area, plot=FALSE)$out
...
# Częstość posiadania ogrzewania wodą
table(dane$hotwaterheating)
# Średnia cena dla posiadania ogrzewania wodą
aggregate(dane$price, by = list(dane$hotwaterheating), FUN = mean)
# Wykres pudełkowy dla ceny w zależności od posiadania ogrzewania wodą
boxplot(dane$price ~ dane$hotwaterheating, main="Ceny w zależności od ogrzewania wodą", xlab="Ogrzewanie wodą", ylab="Cena", col="lightpink")
